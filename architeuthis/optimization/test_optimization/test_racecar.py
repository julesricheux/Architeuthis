import architeuthis as asb
import architeuthis.numpy as np
import pytest


def test_racecar(N=100, plot=False):
    ...
    v = opti.derivative_of(
        x, with_respect_to=t, derivative_init_guess=1, method="cubic"
    )
    ...
    from architeuthis.numpy.integrate_discrete import (
        integrate_discrete_squared_curvature,
    )

    effort = 0
    effort = 1e-6 * np.sum(integrate_discrete_squared_curvature(f=u, x=t))
    ...
    if plot:
        ...
        import architeuthis.tools.pretty_plots as p
